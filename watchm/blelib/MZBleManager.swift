...
import UIKit
//全局变量，方便调用
let MZBLE = MZBleManager.share
class MZBleManager: NSObject {
    // 配置连接外设的参数，如service，character
    var configuration = MZConfiguration.init(serviceUUID: "FFF0", characteristicNotifyUUID: "FFF2", characteristicWriteUUID: "FFF5", writeType: .withResponse)
    ...
    //MARK: - private
    private var centralManager: MZCentralManager!
    var bluetoothChannel =  MZBleChannel()
    
    //返回可用设备
    func validPeripheral(_ mzPeripheral:MZPeripheral?) -> MZPeripheral? {
        //如果传入设备为空
        guard mzPeripheral == nil else {
            return mzPeripheral
        }
        //则返回已连接设备的第一个设备
    ...
    override init() {
        super.init()
        centralManager = MZCentralManager()
    }
    ...
    init(queue:DispatchQueue = .main, configuration: MZConfiguration , options:[String:Any]? = nil) {
        super.init()
        centralManager = MZCentralManager(queue: queue, options: options)
    ...
    func blockOnPeripheralStateChange(_ channel: String? = nil,block: @escaping MZPeripheralStateChangeBlock)  {
        bluetoothChannel.currentChannelCallback().blockOnPeripheralStateChange = block
    }
    
    /// 发现新外设
    func blockOnNewPeripheral(_ channel: String? = nil,block: @escaping MZNewPeripheralBlock)  {
        bluetoothChannel.currentChannelCallback().blockOnNewPeripheral = block
    }
    
    /// 新外设已就绪，可以接收数据
    func blockOnPeripheralReady(_ channel: String? = nil,block: @escaping MZPeripheralReadyBlock)  {
        bluetoothChannel.currentChannelCallback().blockOnPeripheralReady = block
    }

    /// 蓝牙外设收到的数据
    func blockOnBluetoothReadData(_ channel: String? = nil,block: @escaping MZbluetoothReadDataBlock)  {
    ...
    func scan(withDuration duration: TimeInterval? = nil,
              ...
              progressHandler: MZScanProgressHandler? = nil,
              completionHandler: MZScanCompletionHandler? = nil)
    {
    ...
    /// 返回当前连接的所有蓝牙外设
    func findConnectedPeriphrals() -> [MZPeripheral] {
    ...
    func connect(mzPeripheral:MZPeripheral? = nil) {
        centralManager.connect(peripheral: mzPeripheral)
    }
    ...
    func autoConnect(peripheralName: String, scantime: TimeInterval = 5, completionHandler: MZConnectPeripheralCompletionHandler? = nil){
    ...
    func sendData(peripheral:MZPeripheral? = nil,
                   data:Data,
                   serviceUUID: String? = nil,
                   writeUUID:String? = nil,
                   type:CBCharacteristicWriteType? = nil,
                   completionHandler: MZSendDataCompletionHandler? = nil)
    {
        guard let sendPeripheral = validPeripheral(peripheral), sendPeripheral.peripheral.state == .connected else{
            //设备未连接
            completionHandler?(peripheral, data, MZError.notConnectPeripheral)
            return
        }

        assert(writeUUID != nil || MZBLE.configuration.characteristicWriteUUID != nil , "characteristicWriteUUID Could not be nil")
        //向设备写入数据
    ...
    func notify(peripheral:MZPeripheral? = nil,serviceUUID: String? = nil,notifyUUID:String? = nil, completionHandler: MZNotifyCharacteristicCompletionHandler?) {
        ...
        assert(notifyUUID != nil || MZBLE.configuration.characteristicNotifyUUID != nil , "characteristicWriteUUID Could not be nil")
        // 订阅设备
